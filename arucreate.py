...
# creates a custom aruco dictionary (_aruco_dict) with n aruco markers in marker size s _aruco_needed), labeled with label_1,_2,_3,_4
# LABELS MÜSSEN NOCH DYNAMISCH WERDEN ; SIND NICHT IMMER 4
    def __init__(self, n=4, s=4, label_1="front", label_2="left", label_3="right", label_4="back"):
        if s == 4:
        ...
        self._aruco_dict = aruco.Dictionary_create_from(n, s, dict)
        self._aruco_needed = n

        self._aruco_1 = label_1
        self._aruco_2 = label_2
        self._aruco_3 = label_3
        self._aruco_4 = label_4

        self.show_arucos()

# ----------------------------------------------------------------------------------------------------------------------
# creates a custom aruco dictionary (class variable _aruco_dict) with optional n = 4 aruco markers in marker size optional s = 4 (class variable _aruco_needed)
    def create_aruco_dict(n=4, s=4):

        if s == 4:
            dict = aruco.getPredefinedDictionary(aruco.DICT_4X4_50)
        elif s == 5:
            dict = aruco.Dictionary_get(aruco.DICT_5X5_50)
        elif s == 6:
            dict = aruco.Dictionary_get(aruco.DICT_6X6_50)
        elif s == 7:
            dict = aruco.Dictionary_get(aruco.DICT_7X7_50)
        else:
            print("no useful marker size entered! Choose marker size 4, 5, 6 or 7")
            return

        _aruco_dict = aruco.Dictionary_create_from(n, s, dict)
        _aruco_needed = n
        #show_arucos()
...
# find aruco marker i with its label and displays it
    def lookup_aruco_nr(self, i):
        if i == 1:
            frame = py_mpl.figure("Aruco Nr 1 -" + self._aruco_1)
        elif i == 2:
            frame = py_mpl.figure("Aruco Nr 2 -" + self._aruco_2)
        elif i == 3:
            frame = py_mpl.figure("Aruco Nr 3 -" + self._aruco_3)
        elif i == 4:
            frame = py_mpl.figure("Aruco Nr 4 -" + self._aruco_4)
        else:
...
# detects markers from respective custom dictionary in an image or captured video and prints out the information
    def lookup_aruco_img(self):
        ...
        while(True):
            ...
            # GREY DOES NOT SEEM TO BE A GOOD IDEA
    ...
    def save_aruco_images(self):
        ...
        if isinstance(self._aruco_needed, int):
            for i in range(0, self._aruco_needed):
                img = aruco.drawMarker(self._aruco_dict, i, 500)
                i_str = str(i+1)
                imageio.imwrite("_markers/marker_"+i_str+".jpg", img)
                print("Marker", i_str, "ist gespeichert.")
